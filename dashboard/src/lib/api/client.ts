...
class ApiClient {
  ...
  async updatePassword(data: UpdatePasswordRequest) {
    return this.patch<void>('/v1/auth/password', data);
  }
  ...
  async updateUserPassword(id: string, password: string) {
    return this.patch<void>(`/v1/admin/users/${id}/password`, { password });
  }
  ...
  async grantDatabase(role: string, database: string, permissions: string) {
    return this.post<void>(`/v1/roles/${role}/grant-database`, { database, permissions });
  }

  async grantSchema(role: string, schema: string, permissions: string) {
    return this.post<void>(`/v1/roles/${role}/grant-schema`, { schema, permissions });
  }

  async grantTable(role: string, schema: string, table: string, permissions: string) {
    return this.post<void>(`/v1/roles/${role}/grant-table`, { schema, table, permissions });
  }

  async revokeDatabase(role: string, database: string, permissions: string, cascade = false) {
    return this.post<void>(`/v1/roles/${role}/revoke-database`, { database, permissions, cascade });
  }

  async revokeSchema(role: string, schema: string, permissions: string, cascade = false) {
    return this.post<void>(`/v1/roles/${role}/revoke-schema`, { schema, permissions, cascade });
  }

  async revokeTable(role: string, schema: string, table: string, permissions: string, cascade = false) {
    return this.post<void>(`/v1/roles/${role}/revoke-table`, { schema, table, permissions, cascade });
  }

  async addMember(role: string, memberOf: string) {
    return this.post<void>(`/v1/roles/${role}/add-member`, { member_of: memberOf });
  }

  async removeMember(role: string, memberOf: string) {
    return this.post<void>(`/v1/roles/${role}/remove-member`, { member_of: memberOf });
  }

  async listDatabasePrivileges(database: string) {
    const result = await this.get<PgPrivilege[]>(`/v1/roles/privileges/databases?database=${encodeURIComponent(database)}`);
    return Array.isArray(result) ? result : [];
  }

  async listSchemaPrivileges(schema: string) {
    const result = await this.get<PgPrivilege[]>(`/v1/roles/privileges/schemas?schema=${encodeURIComponent(schema)}`);
    return Array.isArray(result) ? result : [];
  }

  async listTablePrivileges(schema: string, table: string) {
    const result = await this.get<PgPrivilege[]>(`/v1/roles/privileges/tables?schema=${encodeURIComponent(schema)}&table=${encodeURIComponent(table)}`);
    return Array.isArray(result) ? result : [];
  }
  ...
  async getActiveSessions() {
    const result = await this.get<ActiveSession[]>('/v1/monitoring/sessions');
    return Array.isArray(result) ? result : [];
  }

  async getSlowQueries(minDurationMs = 1000) {
    const result = await this.get<SlowQuery[]>(`/v1/monitoring/slow-queries?min_duration_ms=${minDurationMs}`);
    return Array.isArray(result) ? result : [];
  }

  async getLocks() {
    const result = await this.get<LockInfo[]>('/v1/monitoring/locks');
    return Array.isArray(result) ? result : [];
  }

  async getWaitingQueries() {
    const result = await this.get<WaitingQuery[]>('/v1/monitoring/waiting');
    return Array.isArray(result) ? result : [];
  }

  async getQueryStats(limit = 50) {
    const result = await this.get<QueryStats[]>(`/v1/monitoring/query-stats?limit=${limit}`);
    return Array.isArray(result) ? result : [];
  }

  async getConnectionStats() {
    return this.get<ConnectionStats>('/v1/monitoring/connections');
  }

  async getCacheStats() {
    return this.get<CacheStats>('/v1/monitoring/cache');
  }

  async getDatabaseStats() {
    const result = await this.get<DatabaseStat[]>('/v1/monitoring/databases');
    return Array.isArray(result) ? result : [];
  }

  async getTableStats(schema?: string) {
    const path = schema ? `/v1/monitoring/table-stats?schema=${schema}` : '/v1/monitoring/table-stats';
    const result = await this.get<TableStat[]>(path);
    return Array.isArray(result) ? result : [];
  }

  async getIndexStats(schema?: string) {
    const path = schema ? `/v1/monitoring/index-stats?schema=${schema}` : '/v1/monitoring/index-stats';
    const result = await this.get<IndexStat[]>(path);
    return Array.isArray(result) ? result : [];
  }

  async terminateSession(pid: number) {
    return this.post<void>('/v1/monitoring/sessions/terminate', { pid });
  }
